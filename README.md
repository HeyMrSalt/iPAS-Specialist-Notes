...
# iPAS資安工程師中級 | 防護實務衝刺班 | 證照筆記 | Ver.20250808 [![Hits](https://hits.sh/github.com/heymrsalt/ipas-specialist-notes.svg)](https://hits.sh/github.com/heymrsalt/ipas-specialist-notes/)
...
## 版本更新
> 筆記重大更新都會記錄於此。\
> Ver.20250808 : **Added** 新增 114 年第 1 次【歷屆難題與解析】。\
> Ver.20250312 : **Added** 新增[【用資安新聞學資安名詞】](#用資安新聞學資安名詞)新章節 關注國內外大小資安事件 從中認識些資安名詞。\
> Ver.20250216 : **Changed** 更換筆記 LICENSE
...
### 114-1-Q4
【複選】關於知名密碼破解工具 Hashcat 的說明，下列哪些描述正確？\
(A) 可以破解的密碼種類超過 10 種\
(B) 可以使用 ?u?l?d?d?d?d?d?d 這種 Mask Attack Mode 來進行暴力破解，破解出來的密碼內容範例是 aB123456\
(C) 如果使用參數 -a1 的話，可以同時使用兩個字典檔一起組合破解\
(D) 進行 -a0 字典檔攻擊時，如果使用參數 -r 的話，可以載入一組規則以便進行 Rule-based Attack，例如在所有字典的後面加上 1234 組合成新的密碼\
\
Ans : (A)(C)(D) 或 (C)(D)\
\
原因如下：\
這題其實非常實務，考驗對 Hashcat 這工具的熟悉度，而官方給出兩種答案代表選項(A)存在描述爭議，但我在下方忽略此爭議去解析。\
\
(A) Hashcat 支援數百種雜湊演算法（Hash Types），遠不只 10 種。\
實際上超過 300 種，含 NTLM、bcrypt 等 -help 就會列出完整支援列表。\
*選項(A) 有爭議點可能是此描述過於籠統，用「密碼種類」一詞在技術上或許不夠精確，但這部分我很主觀，再請讀者斟酌參考。\
(B) 明顯錯誤。因為這邊描述的 `?u?l?d...` 代表密碼格式為「大寫字母+小寫字母+數字...」。\
而範例 `aB123456` 的開頭是「小寫+大寫」，不符合該 Mask。\
*選項(B) 改為 `Ab123456` 就是正確的了！\
(C) 參數 `-a1` 代表 **Combinator Attack (組合攻擊)**，這個功能是將兩個字典檔的內容組合起來生成新的密碼候選。\
*例如 `wordlist1.txt` 的 `admin` 和 `wordlist2.txt` 的 `1234` 可以組合成 `admin1234`\
(D) 參數 `-a0` 是字典攻擊模式，而 `-r` (rules-file) 可以載入規則檔。\
規則檔定義了如何變化字典中的單字，例如在每個單字後面加上 `1234` 就是一個常見的規則。

---
</br>

### 114-1-Q9
勒索軟體與資料外洩攻擊之準備、預防、緩解與應變之實務上，可透過分析系統執行特定指令來進行威脅獵補(Threat hunting)，\
以即早偵測並發現異常行為。下列何種 Windows 指令「非」勒索軟體常用於竄改系統檔案/備份之目的？\
(A) fsutil.exe\
(B) ntfsbackup.exe\
(C) vssadmin.exe\
(D) wbadmin.exe\
...
Ans : (B)\
\
這題考的是對勒索軟體常見行為的了解，特別是它們如何破壞系統的備份和還原機制。\
\
(A) `fsutil.exe`：是檔案系統管理工具。勒索軟體有時會利用它來查詢或修改檔案系統屬性，甚至創建大檔案來覆蓋磁碟空間。\
(B) `ntfsbackup.exe`：**這個指令在現代 Windows 作業系統中並不存在**。\
早期的 Windows (如 Windows NT) 有一個 `ntbackup.exe`，但也不是 `ntfsbackup.exe`。\
因此，這是一個攻擊者不會使用的無效指令，是正確答案。\
(C) `vssadmin.exe`：這是**磁碟區陰影複製服務 (Volume Shadow Copy Service)** 的管理工具。這是勒索軟體**最常攻擊的目標之一**。\
它們會執行 `vssadmin.exe delete shadows/all/quiet` 指令來刪除系統的所有還原點，讓受害者無法輕易地還原被加密的檔案。\
(D) `wbadmin.exe`：這是 **Windows Server Backup** 的命令列工具。勒索軟體會使用它來刪除系統備份，斷絕受害者後路。\
\
*了解攻擊者（特別是勒索軟體）如何利用系統「原生工具」來達成惡意目的，是威脅獵捕和防禦的重要一環。\
*`vssadmin` 和 `wbadmin` 的濫用是極為常見的指標 (IOC)。
